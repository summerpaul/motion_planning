 ...
 * @Last Modified time: 2022-09-28 10:03:08
 */
...
void Segment_Expander::Initialize(cv::Mat &_map, cv::Mat &_distField,
                                  cv::Mat &_voronoiPath) {
  nx_ = _map.cols;
  ny_ = _map.rows;
  ns_ = nx_ * ny_;
...
void Segment_Expander::optimizeSegments(std::vector<Segment> &_segments,
                                        float _maxPixelsCrossing,
                                        float _maxPixelsEndSegment) {
  // Move all segments with distance less than _maxPixelCrossing into the
  // crossing
  for (uint32_t i = 0; i < _segments.size(); i++) {
    if (!_segments[i].getOptStart()) {
      optimizeSegmentsAroundPoint(_segments, _segments[i].getStart(),
                                  _maxPixelsCrossing, i);
    }

    if (!_segments[i].getOptEnd()) {
      optimizeSegmentsAroundPoint(_segments, _segments[i].getEnd(),
                                  _maxPixelsCrossing, i);
    }
  }
  ...
  for (uint32_t i = 0; i < _segments.size(); i++) {
    // Its important to have a sorted segment list by id... Therefore we need to
    // update all segments when we remove one of them As well we need to remove
    // all existing neighbors before erasing...
    if ((_segments[i].getStart() - _segments[i].getEnd()).norm() <
            _maxPixelsEndSegment &&
...
void Segment_Expander::removeSegmentFromList(const uint32_t _id,
...
void Segment_Expander::optimizeSegmentsAroundPoint(
    std::vector<Segment> &_segments, const Eigen::Vector2d &pt, float maxPixels,
    int startIndex) {
  ...
  for (uint32_t i = 0; i < connectedSegmnetsStart.size(); i++) {
    ...
    for (uint32_t j = 0; j < connectedSegmnetsEnd.size(); j++) {
      if (!(*connectedSegmnetsStart[i])
        ...
        (*connectedSegmnetsStart[i])
            .addPredecessor((*connectedSegmnetsEnd[j]).getId());
      }
    }
  }
  ...
  for (uint32_t i = 0; i < connectedSegmnetsEnd.size(); i++) {
    for (uint32_t j = 0; j < connectedSegmnetsStart.size(); j++) {
      if (!(*connectedSegmnetsEnd[i])
        ...
        (*connectedSegmnetsEnd[i])
            .addSuccessor((*connectedSegmnetsStart[j]).getId());
      }
    }
    ...
    for (uint32_t j = 0; j < connectedSegmnetsEnd.size(); j++) {
      if (i != j) {
        if (!(*connectedSegmnetsEnd[i])
          ...
          (*connectedSegmnetsEnd[i])
              .addSuccessor((*connectedSegmnetsEnd[j]).getId());
        }
      }
    }
  }
...
Segment_Expander::splitPath(const std::vector<Eigen::Vector2d> &_path,
  ...
  if (_path.size() > 1 && _path.size() <= _minimum_length) {
    ...
    for (uint32_t i = 0; i < nr_segments; i++) {
      ...
      for (uint32_t j = std::max<int>(0, i * real_length);
           j <= (i + 1) * real_length && j < _path.size(); j++) {
...
// 生成图
std::vector<Segment> Segment_Expander::getGraph(
    const std::vector<std::vector<Eigen::Vector2d>> &_endPoints,
    float *_potential, const float _min_length, float _optimizePixelsCrossing,
    const float _optimizePixelsEndSegments) {
  Segment::resetId();

  // We need at least a 10th of max length for seg optimization to find
  // unconnected edges from skeletonizing
  _optimizePixelsCrossing =
      std::max<float>(_min_length / 10, _optimizePixelsCrossing);

  // To be able to change the segments in the crossing class it has to be
  // somehow accessable over the heap -> shared_ptr
  ...
  while (endPoints.size() > 0) {
    ...
    if (startCrossing.size() > 0) {
      ...
      // Reset the potential of each crossing endpoint to prepare for the
      // expansion
      ...
      segments->insert(segments->end(), pathSegments.begin(),
                       pathSegments.end());
  ...
  // Optimize Crossings by moving the center and removing unecessary "double
  // crossings"
  optimizeSegments((*segments), _optimizePixelsCrossing,
                   _optimizePixelsEndSegments);

  return (*segments);
}

float Segment_Expander::getMinSegmentWidth(
       ...
       _path) // auto it = _path.begin(); it != _path.end(); ++it)
  {
    ...
    minimum_path_space =
        std::min<float>(distance_field_[p.i], minimum_path_space);
  }
...
Segment_Expander::calcEndpoints(float *_potential) {
...
bool Segment_Expander::checkSegmentPoint(
    const Segment_Expander::Index &_point) {
  // Check Boundries
...
std::vector<Eigen::Vector2d> Segment_Expander::getPath(
    const Segment_Expander::Index &_start, float *_potential,
    const std::vector<std::vector<Eigen::Vector2d>> &_endpoints) {
  ...
  while (!(queue_.empty()) && (cycle < cycles)) {
    ...
    if (current.i != _start.i && isEndpoint(current, _endpoints)) {
      ...
      addExpansionCandidate(current, current.offsetDist(1, 0, nx_, ny_),
                            _potential);
      addExpansionCandidate(current, current.offsetDist(0, 1, nx_, ny_),
                            _potential);
      addExpansionCandidate(current, current.offsetDist(-1, 0, nx_, ny_),
                            _potential);
      addExpansionCandidate(current, current.offsetDist(0, -1, nx_, ny_),
      ...
      addExpansionCandidate(current, current.offsetDist(1, 1, nx_, ny_),
                            _potential);
      addExpansionCandidate(current, current.offsetDist(-1, 1, nx_, ny_),
                            _potential);
      addExpansionCandidate(current, current.offsetDist(-1, -1, nx_, ny_),
  ...
  if (!findEdgeSegments_ || points.size() <= 3 ||
      std::sqrt(dx * dx + dy * dy) <= 3) {
    points.clear();
  }
...
Eigen::Vector2d Segment_Expander::expandSegment(
    Segment_Expander::Index _start, float *_potential,
    const std::vector<std::vector<Eigen::Vector2d>> &_endpoints) {
  std::vector<Eigen::Vector2d> points;
...
Segment_Expander::expandCrossing(const Index &_start, float *_potential) {
  std::vector<Eigen::Vector2d> points;
...
void Segment_Expander::addExpansionCandidate(const Index &current,
                                             const Index &next,
                                             float *potential) {
...
void Segment_Expander::removeEndpoint(
    const Segment_Expander::Index &_current,
...
bool Segment_Expander::isEndpoint(
    Segment_Expander::Index &_current,
    const std::vector<std::vector<Eigen::Vector2d>> &_endpoints) {
