 ...
 * @Last Modified time: 2022-09-28 10:03:03
 */
...
public:
  ...
  /**
   * @brief initializes the expander by setting the voronoi path map and
   * distancefield
   * @param _map the map
   * @param _distField the distance_field generated from the map (e.g.: opencv
   * distance_transform)
   * @param _voronoiPath the generated voronoi path out of the distance field
   * (e.g.: ridge following; zhang suen thinning...)
   */
   ...
   * @param _endPoints a list of all found endpoints (list of crossings which
   * contain a set of endpoints)
   * @param _potential potential used to expand the pats
   * @param _min_length the minimum length a segment has to have (max =
   * _min_length * 2 - epsilon)
   * @param _optimizePixelsCrossing if crossings have less distance than
   * _optimizePixelsCrossing to each other they are merged
   * @param _optimizePixelsEndSegments if a end segment has less length than
   * this var it is removed
   * @return a list of segments representing the graph
   */
  std::vector<Segment>
  getGraph(const std::vector<std::vector<Eigen::Vector2d>> &_endPoints,
           float *_potential, const float _min_length,
...
private:
  ...
  splitPath(const std::vector<Eigen::Vector2d> &_path,
  ...
  void removeSegmentFromList(const uint32_t _id,
                             std::vector<Segment> &_segments);

  // Adds an expansion candidate for the Dijkstra algorithm
  void addExpansionCandidate(const Index &current, const Index &next,
                             float *potential);
  ...
  // Optimizes the Graph by removing to short segments at the end of it and
  // merging crossings in a specific radius
  void optimizeSegments(std::vector<Segment> &_segments,
                        float _maxPixelsCrossing, float _maxPixelsEndSeg);

  // Merges crossings together
  void optimizeSegmentsAroundPoint(std::vector<Segment> &_segments,
                                   const Eigen::Vector2d &pt, float maxPixels,
                                   int startIndex);
};
